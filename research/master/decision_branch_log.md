...
## DEC-029
`CORE v0.27 Hierarchical Repeated-Trial Execution Gate 0.1` returns `SCIENTIFIC FAIL` at `research/core/v0_27_hierarchical_repeated_trial_execution_gate_0_1.md`. MASTER accepts the result as a valid governed negative result and establishes `CORE v0.27 Hierarchical Repeated-Trial Result Freeze 0.1` / RB-015. C2-27-01 through C2-27-08 all PASS. All 32 immutable datasets are generated with zero insufficient-mask datasets and zero truth physical/chart-invalid datasets. The conservative nuisance-profiled shared-parameter information audit is positive in 32/32 datasets, with condition numbers below the frozen threshold. Nevertheless the preregistered Laplace inference route fails its hard branch criteria: all 96 frozen outer starts have non-finite complete initial objectives because at least one trial-mode BFGS solve hits the frozen `STRONG_WOLFE_20_FAIL`; consequently `OUTER_OPT_FAIL=32/32`, usable fits `0/32`, and valid five-parameter uncertainty intervals `0/32`. The result is SCIENTIFIC FAIL, not CONDITIONAL, and must not be rescued or relabelled. The 32 observed datasets may be used only as post-hoc development evidence in a new method-development branch; any redesigned method selected using them requires a disjoint previously ungenerated confirmatory seed set before performance can be claimed. The single next scientific action is `CORE v0.27b Inference Failure Mechanism & Follow-up Preregistration Gate 0.1`, restricted to diagnosing the failure and preregistering at most one follow-up route. No redesigned confirmatory execution, v0.28 effect-bearing work, active pulse/probe extension, application work, novelty positioning or manuscript claim work is authorized before return.
Status: STABLE
Date: 2026-09-07
